...
const _isVideo = (props) => props.message.type.match(/^video\/.+$/)
...
export class MessageBubble extends Component {
  ...
  _renderMessageItem = (message) => {
    ...
    } if (_isImage(this.props)) {
      return <ImageItem isUser={this.props.isUser} message={message.url.replace('http://', 'https://')} />
    } if (_isVideo(this.props)) {
      return (
        <View style={styles.videoContainer}>
          <Video source={{ uri: message.url }} style={styles.video} paused controls />
        </View>
      )
    }
